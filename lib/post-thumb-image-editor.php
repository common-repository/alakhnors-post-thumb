...
class ImageEditor {
	...
	function ImageEditor($filename=0, $path=0, $col=NULL, $settings='', $is_remote=true) {
		...
		$this->unsharp = (get_pt_options('unsharp') == 'true');

                // Load options
		if (get_pt_options('unsharp_amount') == 0)
	       	 	$this->unsharp_amount = 80;
	        else
	                $this->unsharp_amount = get_pt_options('unsharp_amount');

		if (get_pt_options('unsharp_radius') == 0)
	        	$this->unsharp_radius = 0.5;
	        else
	                $this->unsharp_radius = get_pt_options('unsharp_radius');

		if (get_pt_options('unsharp_threshold') == 0)
	        	$this->unsharp_threshold = 3;
	        else
	                $this->unsharp_threshold = get_pt_options('unsharp_threshold');

		if (get_pt_options('corner_ratio') == 0)
	        	$this->corner_ratio = 0.15;
	        else
	                $this->corner_ratio = get_pt_options('corner_ratio');
...
function setMemoryForImage($filename, $TWEAKFACTOR = 1.7, $curl = false, $filesize=0 ) {
	...
	//Default memory limit is usually.
...
function ImageColorAllocateAlphaSafe(&$gdimg_hexcolorallocate, $R, $G, $B, $alpha=false) {
	...
	if (version_compare(phpversion(), '4.3.2', '>=') && ($alpha !== false)) {
