...
class pt_thumbnail {
	...
	function get_thumbnail() {
		...
		// Save image info
		$the_processed_image = $this->imageFilter();                
                $is_remote  = ($this->image == $the_processed_image);                                                                      // Fix for Dreamhost
		$dest_path = pt_pathinfo($the_processed_image);
		...
		if (file_exists($save_dir.'/'.$rename_to) && (!$this->base_name)) {
                        $this->thumb_url 	= $save_url.'/'.$rename_to;
			$this->thumb_path   	= $save_dir.'/'.$rename_to;
		}

		// check if image exists. If doesn't exist, can't do anything so return false
		...
		// if file has to be generated, generates thumbnails
			...
			// Create the image
			$thumb = new ImageEditor($the_processed_image,'', '', $this->settings, $is_remote);          // Fix for Dreamhost
	...
	function ImageFilter() {
		...
		$this->the_image = str_replace ('%20',' ', $this->the_image);
		$url = str_replace ('&amp;','&', $this->the_image);

		// Filters ngg singlepic
		if (stripos($url, "nggshow.php")) {
			...
			if ($ngg_pic) {
                ...
                $the_processed_image = htmlentities(str_replace($this->settings['full_domain_name'], $this->settings['base_path'], $this->the_image));   // Fix for Dreamhost
...
class pt_post_thumbnail {
	...
	/* Constructor
	/****************************************************************/
	function pt_post_thumbnail ($settings, $post='', $arg='', $default_image='', $default_media='', $permalink='', $title='', $date='', $author='', $link='', $search=true, $vwidth=0, $vheight=0) {
	...
	function GetImgHTML() {
		...
		if ($this->LB_effect && $this->use_hs) {
			...
			elseif ($this->has_media) {
				...
				if ($this->settings['wordtube_pheight'] == 0) 
					$vheight = $this->settings['wordtube_pwidth'] * $this->vheight / $this->vwidth;
				else
					$vheight = $this->settings['wordtube_pheight'];
				$this->html = $this->GetVideo ($this->title, $this->the_default_media, $this->thumb_url, $this->settings['wordtube_pwidth'], $vheight);
